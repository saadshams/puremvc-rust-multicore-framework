...
fn test_compare_notify_context() {
    ...
    let neg_context: Arc<dyn Any + Send + Sync> = Arc::new(neg_controller.clone());
...
fn test_compare_notify_context2() {
    ...
    let observer = Observer::new(None, Some(Arc::clone(&context)));
...
fn test_compare_notify_context3() {
    ...
    assert_eq!(observer.compare_notify_context(&(context as Arc<dyn Any + Send + Sync>)), false); // unsupported type
}
